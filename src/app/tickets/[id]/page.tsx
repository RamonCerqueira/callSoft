...
import { api, Ticket, Cotacao } from "../../../lib/api";
import { ArrowLeft, Clock, User, Mail, Phone, Calendar, Tag, AlertCircle, FileText, Plus, ShoppingCart, Building2, MessageCircle, CheckCircle2, XCircle, Info } from "lucide-react";
...
export default function TicketDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: historyData, isLoading: isLoadingHistory, refetch: refetchHistory } = useQuery<{ success?: boolean; data?: Array<{ id: string; userName: string; status: string; timestamp: string; note?: string }> }>({
    queryKey: ["ticket-history", params.id],
    queryFn: async () => {
      try {
        const res = await api.get(`/api/v1/tickets/${params.id}/history`);
        return res.data;
      } catch {
  ...
  const transitionStatus = async (status: Ticket["status"]) => {
    try {
      await api.post(`/api/v1/tickets/${params.id}/status`, { status });
      addNotification({
                        ...
                        <Card variant="glass">
                            <CardHeader className="flex flex-row items-start justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {ticket.clientPhone && (
                                        <Button variant="ghost" size="icon" onClick={() => handleWhatsApp(ticket.clientPhone)} title="WhatsApp">
                                            <MessageCircle className="h-4 w-4 text-green-400" />
                                        </Button>
                                    )}
                                    <Button variant="ghost" size="icon" onClick={() => transitionStatus("ABERTO")} title="Aprovar">
                                        <Info className="h-4 w-4 text-blue-400" />
                                    </Button>
                                    <Button variant="ghost" size="icon" onClick={() => transitionStatus("RESOLVIDO")} title="Finalizar">
                                        <CheckCircle2 className="h-4 w-4 text-green-400" />
                                    </Button>
                                    <Button variant="ghost" size="icon" onClick={() => transitionStatus("CANCELADO")} title="Cancelar">
