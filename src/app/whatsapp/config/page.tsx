...
const DEFAULT_CONFIG: ChatbotConfig = {
    ...
    triggerKeywords: "oi, olá, menu, ajuda, iniciar",
    welcomeMessage: "Olá! Sou o assistente virtual da CallSoft. Como posso ajudar você hoje? Escolha uma das opções abaixo:",
    fallbackMessage: "Desculpe, não entendi. Por favor, digite o número da opção desejada ou 'menu' para voltar ao início.",
    menuOptions: [
        { id: '1', trigger: '1', label: 'Suporte Técnico', action: 'message', response: 'Para suporte técnico, por favor descreva seu problema brevemente.' },
        { id: '2', trigger: '2', label: 'Financeiro', action: 'transfer', response: 'Aguarde um momento, estou transferindo para o setor financeiro...' },
        { id: '3', trigger: '3', label: 'Site Oficial', action: 'link', response: 'https://callsoft.com.br' }
    ],
    businessHours: {
        enabled: false,
        start: "08:00",
        end: "18:00",
        outOfHoursMessage: "Nosso horário de atendimento é das 08:00 às 18:00. Deixe sua mensagem e retornaremos em breve.",
...
export default function WhatsAppConfigPage() {
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            const saved = localStorage.getItem("wpp_chatbot_config_v2");
            if (saved) {
                try {
    ...
    const handleAddOption = () => {
        const newOption: MenuOption = {
            ...
            trigger: (config.menuOptions.length + 1).toString(),
            label: "Nova Opção",
            action: "message",
            response: ""
        };
    ...
    const save = () => {
        localStorage.setItem("wpp_chatbot_config_v2", JSON.stringify(config));
